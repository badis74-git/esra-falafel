...
import type { RestaurantReview } from '@/lib/mock/restaurants'
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex-1 space-y-5 p-5">
        <h3 className="text-sm font-semibold text-primary">{t('reviews.title')}</h3>
        ...
        <div className="flex items-start gap-6">
          <div className="flex flex-col items-center">
            ...
            <span className="text-xs text-neutral-500 mt-1">({reviews.length} {t('reviews.reviewsCount')})</span>
          </div>
        ...
        <div className="space-y-4">
          {reviews.map((review) => (
            <div key={review.id} className="flex gap-3">
              <Avatar name={review.reviewer} src={review.avatar || undefined} size="sm" />
